...
import { usePrimaryAccount } from './accounts/scope'

import { checkIfCustomDisplayNameIsValid } from '../lib/validations/properties'
import { parseCustomDisplayName } from '../lib/utils'

/**
 * The scope's subject. Kept under its old name so the dozen call sites that
 * only ever wanted "the current account" did not all have to change.
 */
export function useGetSelectedAccount() {
  return { selected: usePrimaryAccount() }
}
