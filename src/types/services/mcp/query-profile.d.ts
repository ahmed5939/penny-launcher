...
export type MCPQueryProfileChanges = {
  ...
  profile: {
    ...
    items: Record<
      ...
      | MCPQueryProfileProfileChangesCollectedResource
      | MCPQueryProfileProfileChangesExpedition
    >
  }
}

/**
 * The urn a resource collector fills up — research points and the like.
 * Claimed with `ClaimCollectedResources`, passing this item's GUID.
 */
export type MCPQueryProfileProfileChangesCollectedResource = {
  templateId: `CollectedResource:${string}`
  attributes: Partial<{
    item_seen: boolean
    level: number
    /** How much is banked and waiting to be claimed. */
...
/**
 * Attributes here are modelled loosely on purpose: Epic ships slots in
 * several shapes (empty slot, in-flight, ready to collect) and the endpoint
 * documentation does not pin the item down.
 */
export type MCPQueryProfileProfileChangesExpedition = {
  templateId: `Expedition:${string}`
  attributes: Partial<{
    expedition_expiration_end_time: string
    expedition_criteria: Array<string>
    expedition_max_target_power: number
    expedition_min_target_power: number
    expedition_success_chance: number
    expedition_squad_id: string
    expedition_slot_id: string
    /** Present once the expedition is running; ISO date it finishes. */
    expedition_end_time: string
    expedition_start_time: string
    item_seen: boolean
    level: number
  }>
  quantity: number
