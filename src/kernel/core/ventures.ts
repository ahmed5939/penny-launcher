...
import { parseVenturesProfile } from '../../features/ventures/model'

/** Read-only. Account id in, main-process account lookup, no token leaves this file. */
export async function requestVentures(accountId: string) {
  ...
  if (!response.ok) throw new Error(`Could not load Ventures progress (HTTP ${response.status}). Try Refresh.`)
  return parseVenturesProfile(await response.json(), accountId)
}
