...
import { patchWithTocRepair } from './utoc'
...
/**
 * Write the two height bytes, then repair the .utoc chunk hash so the
 * container stays internally consistent — throws (leaving the original
 * bytes restored) when the TOC cannot be repaired.
 */
function patchBytes(
  filePath: string,
  position: number,
  ...
  const original = readBytesAt(filePath, position, 2)
  const result = patchWithTocRepair(
    filePath,
    position,
    Buffer.from([b0, b1]),
    original
