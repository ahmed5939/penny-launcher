...
describe('reviewed plugin lifecycle', () => {
  ...
  it('revokes host features and cleans up on disable, safe mode and removal', async () => {
    expect(manager.isRunning('sample')).toBe(false)
    await install()
    const cleanup = vi.fn(() => expect(manager.isRunning('sample')).toBe(false))
    manager.registerHostCleanup('sample', cleanup)
    expect(manager.isRunning('sample')).toBe(true)
    await manager.manage({ action: 'disable', id: 'sample' })
    expect(cleanup).toHaveBeenCalledTimes(1)
    await manager.manage({ action: 'enable', id: 'sample' })
    expect(manager.isRunning('sample')).toBe(true)
